...
<html>
	<head>
		<script type="text/javascript" charset="utf-8">
			...
			function agregar(){
				document.form.action="categoriaart.action?estado=<%=ConstantesAplicativo.constanteEstadoAdd%>";
				document.form.submit();
			}
			
			function registrar(){
				document.form.action="categoriaart.action?estado=<%=ConstantesAplicativo.constanteEstadoSave%>";
				document.form.submit();
			}
			
			function volver(){
				document.form.action="categoriaart.action?estado=<%=ConstantesAplicativo.constanteEstadoAll%>";
				document.form.submit();
			}
			
			function modificar(param){
				document.form.action="categoriaart.action?estado=<%=ConstantesAplicativo.constanteEstadoEdit%>&id="+param;
				document.form.submit();
			}
			
			function detalle(param){
				document.form.action="categoriaart.action?estado=<%=ConstantesAplicativo.constanteEstadoAbstract%>&id="+param;
				document.form.submit();
			}
		...
		<s:form id="form">
			...
			<div id="demo">
				...
				<s:if test="estado=='all'||estado=='query'">
					<table cellpadding="0" cellspacing="0" border="0" class="display" id="categoriaArticulo">
						<thead>
							<tr>
								<s:if test="estado=='all'">
								...
								<th><s:text name="categoriaart.nombre"></s:text></th>
								<th><s:text name="categoriaart.descripcion"></s:text></th>
								<th><s:text name="categoriaart.estado"></s:text></th>
								<th><s:text name="global.fecharegistra"></s:text></th>
							</tr>
						</thead>
						<tbody>
							<s:iterator value="listCategoria" id="data">
								<tr>
									<s:if test="estado=='all'">
										<td align="center">
									...
									<td><s:property value="cateDescripcion"/></td>
									<td><s:if test="cateEstado==\"S\"">Activo</s:if><s:else>Inactivo</s:else></td>
									<td><s:property value="cateFechacambio"/>&nbsp;<s:property value="cateHoracambio"/></td>
								</tr>
							</s:iterator>
						</tbody>
						<tfoot></tfoot>
				...
				<s:elseif test="estado=='add'||estado=='edit'||estado=='save'">
					<table cellpadding="0" cellspacing="0" border="0" class="display">
						<s:hidden name="categoria.cateId" id="cateId"></s:hidden>
						<tr>
