...
/**
 * The DAO class for the Virtuallink entity.
 */
@Repository
public class VirtualLinkDAOImpl implements IVirtualLinkDao {
	@PersistenceContext(unitName="entityManagerFactoryPostgresOptica")
    private EntityManager em;

	public VirtualLinkDAOImpl() {
		super();
	}
	/**
	...
	public Virtuallink findVirtuallinkById(long id) {
		return (Virtuallink)em.find(Virtuallink.class, new Long(id));
	}
	/**
	...
	public List<Virtuallink> findAllVirtuallinks() {
        try {
    		String jpqlString = "select virtuallink from " + Virtuallink.class.getSimpleName() + " virtuallink";
	...
	public void persistVirtuallink(Virtuallink virtuallink) {
		em.persist(em.merge(virtuallink));
	}
	/**
	...
	public void removeVirtuallink(Virtuallink virtuallink) {
		...
		em.remove(em.merge(virtuallink));
	}
}